...
updateIncome(id: number, updatedIncome: Income): Observable<Income[]> {
 return this.http.put<Income[]>(`${this.url}/${id}`, updatedIncome);
}

deleteIncome(id: number): Observable<Income[]> {
 return this.http.delete<Income[]>(`${this.url}/${id}`);
}
